	...
	*/
	public ItemStack onItemUseFinish(ItemStack stack, World worldIn, EntityPlayer player) {
		...
		if (props.regenMana(strength)) {
			player.addChatMessage(new ChatComponentText("Mana Restored" + ":" + strength));
			player.addChatMessage(new ChatComponentText("CurrentMana" + ":" + props.getCurrentMana()));
		}
		else {
			player.addChatMessage(new ChatComponentText("Mana full, no need to use this potion!!!"));
		}
		return stack;
	}
